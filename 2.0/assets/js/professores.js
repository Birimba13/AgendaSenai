...
async function carregarProfessores() {
    ...
    try {
        // Mostra loading
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px;"><div style="font-size: 2rem; color: #0a2342;">⏳</div><br>Carregando professores...</td></tr>';
        ...
        if (result.success && result.data.length > 0) {
            ...
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px;">Nenhum professor encontrado. <button class="btn-acao btn-editar" onclick="abrirModal()" style="margin-top: 10px;">Adicionar Primeiro Professor</button></td></tr>';
        }
    } catch (error) {
        console.error('Erro ao carregar professores:', error);
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px; color: #dc2626;">⚠️<br><br>Erro ao carregar professores. Tente novamente mais tarde.</td></tr>';
    }
}
...
function renderizarProfessores(listaProfessores) {
    ...
    if (listaProfessores.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px;">Nenhum professor encontrado.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = listaProfessores.map(prof => `
        <tr>
            ...
            <td>${prof.carga_horaria_usada}h / ${prof.carga_horaria_total}h</td>
            <td>
